...
@php
    ...
    $recommendations = Cache::remember('site.movies.recommendations', setting('site_cache_ttl', 5 * 60), function () {
        return Movie::where('is_recommended', 0)
...
@push('scripts')
    ...
    <script>
        $("#film-hot").owlCarousel({
            ...
            itemsMobile: [479, 2],
            scrollPerPage: true,
            lazyLoad: false, // Tắt lazyLoad mặc định của owlCarousel để sử dụng lazysizes
            navigation: true,
            slideSpeed: 800,
            paginationSpeed: 400,
            stopOnHover: true,
            ...
            // Thêm callback sau khi owl-carousel rendered
            afterInit: function() {
        ...
        $("#film-hot").on('translated.owl.carousel', function(event) {
            if (window.lazySizes) {
                lazySizes.init();
            }
        });
