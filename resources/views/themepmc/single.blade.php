...
@section('content')
    <div class="container" id="detail-page">
        ...
        <div class="film-info" itemscope itemtype="https://schema.org/Movie">
            <!-- Tối ưu background với data-bg và lazysizes -->
            <div class="image lazyload" data-bg="{{ \App\Helpers\OptimizedImage::get($currentMovie->getPosterUrl(), 'large') }}">
                <!-- Tối ưu ảnh phim chính với OptimizedImage helper -->
                <img class="avatar lazyload" itemprop="image" 
                     alt="{{ $currentMovie->name }}"
                     width="300" height="450" 
                     data-src="{{ \App\Helpers\OptimizedImage::get($currentMovie->getThumbUrl(), 'medium') }}"
                     data-srcset="{{ \App\Helpers\OptimizedImage::srcset($currentMovie->getThumbUrl()) }}"
            ...
            <div class="text">
                <div class="social">
                    ...
                    <div class="box-rating">
                        ...
                        <!-- Tối ưu hóa ảnh schema.org với OptimizedImage helper -->
                        <img class="hidden lazyload" itemprop="thumbnailUrl"
                            data-src="{{ \App\Helpers\OptimizedImage::get($currentMovie->getPosterUrl(), 'medium') }}"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                            ...
                            $(document).ready(function() {
                                ...
                                function load_biplayer() {
                                    playerInstance.setup({
                                        ...
                                        image: "{{ \App\Helpers\OptimizedImage::get($currentMovie->getPosterUrl(), 'medium') }}",
                                        skin: {
                                            name: "seven",
                    ...
                    @foreach ($movie_related as $movie)
                        <li class="item ">
                            ...
                            <a title="{{ $movie->name }} - {{ $movie->origin_name }}" href="{{ $movie->getUrl() }}">
                                <!-- Tối ưu hóa ảnh với OptimizedImage helper -->
...
@push('scripts')
    ...
    <script>
        $(function() {
            // Cập nhật cấu hình Owl Carousel để hỗ trợ tốt hơn với lazysizes
            $("#list-film-realted").owlCarousel({
                ...
                pagination: false,
                autoPlay: 8000,
                lazyLoad: false, // Tắt lazyLoad mặc định của Owl Carousel
                navigationText: ['<i class="fa fa fa-caret-left"></i>',
                    '<i class="fa fa fa-caret-right"></i>'
                ],
                afterInit: function() {
                    // Kích hoạt lazysizes sau khi carousel được khởi tạo
                    if (window.lazySizes) {
                        lazySizes.init();
                    }
                },
                afterUpdate: function() {
                    // Kích hoạt lazysizes sau khi carousel cập nhật
                    if (window.lazySizes) {
                        lazySizes.init();
                    }
                }
            ...
            // Kích hoạt lazysizes khi carousel chuyển slide
            $("#list-film-realted").on('translated.owl.carousel', function(event) {
                if (window.lazySizes) {
                    lazySizes.init();
                }
            });
